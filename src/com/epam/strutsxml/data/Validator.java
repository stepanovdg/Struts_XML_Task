...
public class Validator {
    ...
    public static String validate(Goods goods) {
        StringBuilder stringBuilder = new StringBuilder();
        if (!validateName(goods.getUnit())) stringBuilder.append(NAME_MESSAGE);
        if (!validateProducer(goods.getProvider())) stringBuilder.append(PRODUCER_MESSAGE);
        if (!validateModel(goods.getModel())) stringBuilder.append(MODEL_MESSAGE);
        if (!validateDate(goods.getDateOfIssue())) stringBuilder.append(DATE_MESSAGE);
        if (!validateColor(goods.getColor())) stringBuilder.append(COLOR_MESSAGE);
        if (!validatePrice(goods.getPrice().toString())
                && !Boolean.valueOf(goods.getStock()))
            stringBuilder.append(PRICE_MESSAGE);
